...
def fetch_matches_europe(today_br, today_iso):
    ...
    msg = client.messages.create(
        ...
        messages=[{"role": "user", "content": f"CRITICAL: Today is {today_br} ({today_iso}). Search the web for football matches happening TODAY {today_iso} in these competitions: Premier League, Bundesliga, La Liga, Serie A, Ligue 1, Eredivisie, Champions League, Europa League, Conference League. Search 'football fixtures {today_iso}' and 'soccer today {today_iso}'. Only include matches confirmed for {today_iso}. For each match return JSON with fields: home, away, league, time (BRT HH:MM), mg_casa (home avg goals this season 0.5-3.5), mg_fora (away avg goals 0.5-3.5), ov_casa (% home games over 3.5 goals 10-80), ov_fora (% away games over 3.5 goals 10-80), h2h (of last 5 H2H how many had over 3.5 goals 0-5), importance (0=meaningless 1=normal 2=title/relegation/cup), note (one sentence on goal potential). Return ONLY the JSON array starting with [ and ending with ]."}]
    )
...
def fetch_matches_brazil(today_br, today_iso):
    ...
    msg = client.messages.create(
        ...
        tools=[{"type": "web_search_20250305", "name": "web_search"}],
        system="You are a football data assistant. You MUST respond with ONLY a valid JSON array. Every response must start with [ and end with ]. Never add any text outside the JSON array.",
        messages=[{"role": "user", "content": f"CRITICAL: Today is {today_br} ({today_iso}). Search the web for Brazilian football matches happening TODAY {today_iso}: Brasileirao Serie A, Serie B, Copa do Brasil, Estaduais. Search 'futebol hoje {today_br}' and 'jogos hoje {today_br}'. Only include matches confirmed for {today_iso}. For each match return JSON with fields: home, away, league, time (BRT HH:MM), mg_casa (home avg goals this season 0.5-3.0), mg_fora (away avg goals 0.5-3.0), ov_casa (% home games over 3.5 goals 10-60), ov_fora (% away games over 3.5 goals 10-60), h2h (of last 5 H2H how many had over 3.5 goals 0-5), importance (0=meaningless 1=normal 2=title/relegation/cup), note (one sentence on goal potential). Return ONLY the JSON array starting with [ and ending with ]."}]
    )
    raw = "".join(b.text for b in msg.content if b.type == "text")
